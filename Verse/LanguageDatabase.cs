...
namespace Verse
{
	public static class LanguageDatabase
	{
		...
		public static void InitAllMetadata()
		{
			foreach (ModContentPack runningMod in LoadedModManager.RunningMods)
			{
				...
				foreach (string item2 in runningMod.foldersToLoadDescendingOrder)
				{
					...
					{
						if (!directory.完整路径.StartsWith(text))
						{
							Log.Error("Failed to get a relative path for a file: " + directory.完整路径 + ", located in " + text);
							continue;
						}
						string item = directory.完整路径.Substring(text.Length);
		...
		private static LoadedLanguage InitLanguageMetadataFrom(虚拟目录 langDir)
		{
