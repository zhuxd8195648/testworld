...
namespace Verse
{
	public static class GameDataSaveLoader
	{
		...
		public static void SaveScenario(Scenario scen, string absFilePath)
		{
			try
			{
				scen.fileName = Path.得到单个文件NameWithoutExtension(absFilePath);
		...
		public static bool TryLoadScenario(string absPath, ScenarioCategory category, out Scenario scen)
		{
			...
			try
			{
				...
				scen.fileName = Path.得到单个文件NameWithoutExtension(new FileInfo(absPath).Name);
		...
		{
			LoadGame(Path.得到单个文件NameWithoutExtension(saveFile.Name));
		}
	}
}
