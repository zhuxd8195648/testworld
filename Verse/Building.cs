...
public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
{
    ...
    if (def.IsEdifice())
    {
        ...
        // 如果Fillage属性为Full，则将terrainGrid标记为“脏”的，需要重新绘制
        if (def.Fillage == FillCategory.Full)
        {
            map.terrainGrid.Drawer.SetDirty();
        }
        // 如果AffectsFertility属性为true，则将fertilityGrid标记为“脏”的，需要重新绘制
        if (def.AffectsFertility)
        {
            map.fertilityGrid.Drawer.SetDirty();
        }
    ...
    {
        // 如果设置了MakeFog，则遍历占据矩形中的每个格子，并通知fogGrid移除阻挡云彩
        if (def.MakeFog)
        {
        ...
        // 如果holdsRoof为true，则将该建筑的位置信息和地图作为参数传递给RoofCollapseCellsFinder，并调用其静态方法Notify_RoofHolderDespawned
        if (def.holdsRoof)
        {
            RoofCollapseCellsFinder.Notify_RoofHolderDespawned(this, map);
        }
        // 如果IsSmoothable为true，则将该建筑的位置信息和地图作为参数传递给SmoothSurfaceDesignatorUtility，并调用其静态方法Notify_BuildingDespawned
        if (def.IsSmoothable)
        {
            SmoothSurfaceDesignatorUtility.Notify_BuildingDespawned(this, map);
        }
    }
    // 如果sustainerAmbient不为空，则调用其End方法，停止环境音效
