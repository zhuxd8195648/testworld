		...
		public static IEnumerable<T> GetAllInFolder(string folderPath)
		{
			...
			for (int j = mods.Count - 1; j >= 0; j--)
			{
				for (int i = 0; i < mods[j].assetBundles.loadedAssetBundles.Count; i++)
				{
					...
					if (typeof(T) == typeof(Texture2D))
					{
						string 完整路径 = Path.Combine(Path.Combine(dirForBundle2, GenFilePaths.ContentPath<Texture2D>()).Replace('\\', '/'), folderPath).ToLower();
						IEnumerable<string> enumerable = from p in mods[j].AllAssetNamesInBundle(i)
							where p.StartsWith(完整路径)
							select p;
					...
					string 完整路径2 = Path.Combine(Path.Combine(dirForBundle2, GenFilePaths.ContentPath<AudioClip>()).Replace('\\', '/'), folderPath).ToLower();
					IEnumerable<string> enumerable2 = from p in mods[j].AllAssetNamesInBundle(i)
						where p.StartsWith(完整路径2)
						select p;
