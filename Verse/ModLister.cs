...
namespace Verse
{
	public static class ModLister
	{
		...
		public static void RebuildModList()
		{
			...
			foreach (string item in from d in new DirectoryInfo(GenFilePaths.OfficialModsFolderPath).获取目录()
				select d.FullName)
			{
			...
			foreach (string item2 in from d in new DirectoryInfo(GenFilePaths.ModsFolderPath).获取目录()
				select d.FullName)
			{
