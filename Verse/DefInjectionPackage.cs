...
namespace Verse
{
	public class DefInjectionPackage
	{
		...
		public void AddDataFromFile(虚拟文件 file, out bool xmlParseError)
		{
			xmlParseError = false;
			try
			{
				foreach (XElement item in file.加载为XDocument().Root.Elements())
				{
					if (item.Name == "rep")
					{
		...
		private void TryAddInjection(虚拟文件 file, string key, string translation)
		{
			string text = key;
		...
		private void TryAddFullListInjection(虚拟文件 file, string key, List<string> translation, List<Pair<int, string>> comments)
		{
			string text = key;
			key = BackCompatibleKey(key);
		...
		private bool CheckErrors(虚拟文件 file, string key, string nonBackCompatibleKey, bool replacingFullList)
		{
			if (!key.Contains('.'))
			{
		...
		public List<string> MissingInjections(List<string> outUnnecessaryDefInjections)
		{
			...
			DefInjectionUtility.ForEachPossibleDefInjection(defType, delegate(string suggestedPath, string normalizedPath, bool isCollection, string str, IEnumerable<string> collection, bool translationAllowed, bool fullListTranslationAllowed, FieldInfo fieldInfo, Def def)
			{
				...
				else if (injectionsByNormalizedPath.TryGetValue(normalizedPath, out value2) && value2.IsFullListInjection)
				{
					...
					{
						missingInjections.Add(suggestedPath + (value2.fileSource.NullOrEmpty() ? "" : (" (placeholder 已存在 in " + value2.fileSource + ")")));
					}
				}
				else
				{
					...
					foreach (string item in collection)
					{
						...
						if (flag2 && translationAllowed && DefInjectionUtility.ShouldCheckMissingInjection(item, fieldInfo, def))
						{
							...
							missingInjections.Add(text2 + " '" + item.Replace("\n", "\\n") + "'" + (fullListTranslationAllowed ? " (hint: this list allows full-list translation by using <li> nodes)" : "") + (text3.NullOrEmpty() ? "" : (" (placeholder 已存在 in " + text3 + ")")));
						}
						num++;
					}
				}
