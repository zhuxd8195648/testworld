...
namespace Verse
{
	public static class LanguageDataWriter
	{
		public static void WriteBackstoryFile()
		{
			...
			if (new FileInfo(GenFilePaths.BackstoryOutputFilePath).已存在)
			{
				Find.WindowStack.Add(new Dialog_MessageBox("Cannot write: File already 已存在 at " + GenFilePaths.BackstoryOutputFilePath));
				return;
			}
