	...
	public static class DirectXmlLoader
	{
		...
		public static LoadableXmlAsset[] XmlAssetsInModFolder(ModContentPack mod, string folderPath, List<string> foldersToLoadDebug = null)
		{
			...
			GenThreading.ParallelFor(0, fileList.Count, delegate(int i)
			{
				...
				LoadableXmlAsset loadableXmlAsset = new LoadableXmlAsset(fileInfo2.Name, fileInfo2.Directory.FullName, File.阅读所有的文本(fileInfo2.FullName))
				{
					mod = mod
				};
		...
		{
			if (!new FileInfo(filePath).已存在)
			{
				return new T();
			}
			return ItemFromXmlString<T>(File.阅读所有的文本(filePath), filePath, resolveCrossRefs);
		}

		public static T ItemFromXmlFile<T>(虚拟目录 directory, string filePath, bool resolveCrossRefs = true) where T : new()
		{
			if (!directory.文件已经已存在(filePath))
			{
				return new T();
			}
			return ItemFromXmlString<T>(directory.阅读所有的文本(filePath), filePath, resolveCrossRefs);
		}
