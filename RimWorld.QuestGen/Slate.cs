...
namespace RimWorld.QuestGen
{
	public class Slate
	{
		public struct VarRestoreInfo
		{
			...
			public VarRestoreInfo(string name, bool 已存在, object value)
			{
				this.name = name;
				this.已存在 = 已存在;
				this.value = value;
		...
		public bool 已存在(string name, bool isAbsoluteName = false)
		{
			if (name.NullOrEmpty())
			{
				return false;
		...
		public VarRestoreInfo GetRestoreInfo(string name)
		{
			...
			{
				object var;
				bool 已存在 = TryGet<object>(name, out var);
				return new VarRestoreInfo(name, 已存在, var);
			}
			finally
			{
