...
namespace Verse.AI
{
	...
	{
		public static bool MachinesLike(Faction machineFaction, Pawn p)//像机器一样
		{
		...
		public static bool CanUseItemForWork(Pawn p, Thing item)//工作中可以使用物品吗
		{
		...
		public static bool CanBeArrestedBy(this Pawn pawn, Pawn arrester)//可以被逮捕
		{
		...
		public static bool InDangerousCombat(Pawn pawn)//在危险的战斗中
		{
		...
		public static IntVec3 RandomRaidDest(IntVec3 raidSpawnLoc, Map map)//随机袭击目标
		{
		...
		public static bool EnemyIsNear(Pawn p, float radius)//敌人就在附近
		{
			if (!p.Spawned)
			{
				return false;
