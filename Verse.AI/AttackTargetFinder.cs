...
namespace Verse.AI
{
	public static class AttackTargetFinder
	{
		...
		public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f, IntVec3 locus = default(IntVec3), float 轨迹的最大行程半径 = float.MaxValue, bool canBash = false, bool 可以攻击比有效最小射程更近的目标吗   = true)
		{
			...
			if (verb == null)
			{
				Log.Error("最佳攻击目标 " + searcher.ToStringSafe() + " 谁没有攻击verb.");
				return null;
			}
			bool onlyTargetMachines = verb.IsEMP();
			float minDistSquared = minDist * minDist;
			float num = 轨迹的最大行程半径 + verb.verbProps.range;
			...
			Predicate<IAttackTarget> innerValidator = delegate(IAttackTarget t)
			{
				...
				if (轨迹的最大行程半径 < 9999f && (float)(thing.Position - locus).LengthHorizontalSquared > maxLocusDistSquared)
				{
					return false;
				}
			...
			{
				IAttackTarget attackTarget3 = 找到最佳的近战目标(innerValidator, searcherPawn, maxDist, canBash);
		...
		private static IAttackTarget 找到最佳的近战目标(Predicate<IAttackTarget> validator, Pawn searcherPawn, float maxTargDist, bool canBash)
		{
		...
		public static IAttackTarget BestShootTargetFromCurrentPosition(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDistance = 0f, float maxDistance = 9999f)
		{
			...
			return BestAttackTarget(searcher, flags, validator, Mathf.Max(minDistance, currentEffectiveVerb.verbProps.minRange), Mathf.Min(maxDistance, currentEffectiveVerb.verbProps.range), default(IntVec3), float.MaxValue, canBash: false, 可以攻击比有效最小射程更近的目标吗  : false);
		}
