		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			foreach (SaveFileInfo file in files)
			{
				if (num + vector.y >= scrollPosition.y && num <= scrollPosition.y + outRect.height)
				{
					...
					{
						DoFileInteraction(Path.得到单个文件NameWithoutExtension(file.FileInfo.Name));
					}
					...
					Text.Font = GameFont.Small;
					string fileNameWithoutExtension = Path.得到单个文件NameWithoutExtension(file.FileInfo.Name);
