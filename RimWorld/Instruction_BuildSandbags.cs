	...
	public class Instruction_BuildSandbags : Lesson_Instruction
	{
		...
		protected override float ProgressPercent
		{
			get
			{
				...
				{
					if (TutorUtility.BuildingOrBlueprintOrFrameCenter已存在(sandbagCell, base.Map, ThingDefOf.Sandbags))
					{
						num2++;
					}
					num++;
		...
		{
			GenDraw.DrawFieldEdges(sandbagCells.Where((IntVec3 c) => !TutorUtility.BuildingOrBlueprintOrFrameCenter已存在(c, base.Map, ThingDefOf.Sandbags)).ToList());
