	...
	public class Dialog_Options : Window
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			else
			{
				...
				{
					Find.WindowStack.Add(new Dialog_MessageBox(Path.Get完整路径(GenFilePaths.SaveDataFolderPath)));
				}
				if (listing_Standard.ButtonText("ShowLogFileLocation".Translate()))
				{
					Find.WindowStack.Add(new Dialog_MessageBox(Path.Get完整路径(Application.persistentDataPath)));
				}
			}
		...
		{
			FileInfo[] files = new DirectoryInfo(GenFilePaths.ConfigFolderPath).得到多个文件("*.xml");
