...
namespace RimWorld
{
	public static class LovePartnerRelationUtility
	{
		...
		public static bool LovePartnerRelation已存在(Pawn first, Pawn second)
		{
			if (!first.relations.DirectRelation已存在(PawnRelationDefOf.Lover, second) && !first.relations.DirectRelation已存在(PawnRelationDefOf.Fiance, second))
			{
				return first.relations.DirectRelation已存在(PawnRelationDefOf.Spouse, second);
			}
			return true;
		}

		public static bool ExLovePartnerRelation已存在(Pawn first, Pawn second)
		{
			if (!first.relations.DirectRelation已存在(PawnRelationDefOf.ExSpouse, second))
			{
				return first.relations.DirectRelation已存在(PawnRelationDefOf.ExLover, second);
			}
			return true;
		}
		...
		public static Pawn GetPartnerInMyBed(Pawn pawn)
		{
			...
			{
				if (curOccupant != pawn && LovePartnerRelation已存在(pawn, curOccupant))
				{
					return curOccupant;
				}
			}
		...
		public static Pawn GetMostDislikedNonPartnerBedOwner(Pawn p)
		{
			...
			{
				if (ownedBed.OwnersForReading[i] != p && !LovePartnerRelation已存在(p, ownedBed.OwnersForReading[i]))
				{
